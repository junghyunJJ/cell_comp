...
option_list <- list(
    make_option("--rc", action = "store", default = NA, type = "integer", help = "window width for calculate entropy"),
    make_option("--class", action = "store", default = FALSE, type = "character", help = "homo? hetero?"),
    make_option("--th", action = "store", default = NA, type = "integer", help = "th")
)
...
# rc <- 4
# class <- "homo"
# th <- 20 # NOTE!!!!!!!! minimun res based on the "th"


# read entropy results
rawres <- readRDS(str_glue("res/summary_entropy_window_{rc}.rds"))
...
res <- rawres %>%
    ...
    mutate(col = if_else(grepl("Brain|Spinal cord", annotation), "1", "0")) %>%
...
for (sel in levels(res$type)) {
    ...
    sel_names <- str_glue(rc, "_", sel, "_", class, "_", th)
    cat(sel_names, "\n")

    sel_res <- res %>%
        filter(type == sel) %>%
        group_by(timepoint, type, annotation) %>%
    ...
    f_sel_res <- res %>%
        filter(type == sel) %>%
        filter(annotation %in% sel_res$annotation) %>%
        mutate(th = th) %>%
        mutate(class = class)
    ...
    gg_sel_res <- ggplot(f_sel_res, aes(entropy, reorder_within(annotation, entropy, timepoint, median), fill = col)) +
        theme_bw() +
        geom_boxplot() +
